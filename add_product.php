      ...
      if ($fileError === 0) {
        if ($fileSize < 500000) {
          ...
          if ($productName == '' || $categorySelect == '' || $productDetail == '') {
            ...
            $users = $db->prepare("INSERT INTO movies (name, image, time, cat, category_id) VALUES (:name, :image, :time, :cat)");
...
<body>
    ...
    <div class="row my-4">
      <div class="col">
        <label for="name" class="text-light">Film Naam:</label>
    ...
    <div class="row mb-4">
      <div class="col">
        <label for="detail" class="text-light">Beschrijving:</label>
        <textarea type="text" name="productDetail" class="form-control"></textarea>
      </div>
    </div>
    <div class="row mb-4">
      <div class="col">
        <label for="detail" class="text-light">Image bij film:</label>
        <div class="input-group mb-3">
          <input type="file" name="file_upload" class="form-control">
        </div>
      </div>
    </div>
      <button type="submit" class="btn btn-succes text-light" name="submit">Add Movie</button>
    </form>
    <?php
